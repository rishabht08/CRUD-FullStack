const SQL_QUERIES = {
    GET_USERS : "select user_id as id,user_name as name,user_email as email,user_registered as create_at from users",
    CREATE_USER: "insert into users (user_name , user_email, user_password) values (?,?,?)",
    GET_USER_BY_EMAIL : "select * from users where user_email = ?",
    GET_USER_BY_ID : "select * from users where user_id = ?",
    UPDATE_USER:"update users set user_name = ? , user_password = ? where user_id = ?",
